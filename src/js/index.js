...
console.log(
  `Me llamo ${user.name} ${user.surname}, tengo ${user.age} años, y soy ${user.occupation} me gusta ${user.hobbies[0]} y tambien se ${user.hobbies[1]}, acabo de terminar un ${user.education} y tengo mucho tiempo para ${user.hobbies[2]}, no me suelo alejar mucho de mi direción que es ${user.address.street} ${user.address.city} ${user.address.state}, CP. ${user.address.postalCode}, ${user.address.country}, en mi casa tengo dos mascotas un ${user.pets[0].type} que se llama ${user.pets[0].name} y un ${user.pets[1].type} llamado ${user.pets[1].name}, para mi mala fortuna que me tengo que mudar y no me puedo llevar a mis mascotas, si estas interesado en adoptarlos, te dejo mi info: ${user.phone}, ${user.email}.`
);
...
phrase('Si no estudias acabarás como Victor');
